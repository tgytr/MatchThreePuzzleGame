...
public class Board : MonoBehaviour
{
	...
	IEnumerator SwitchTilesRoutine(Tile clickedTile, Tile targetTile)
	{
		...
		if (targetPiece != null && clickedPiece != null)
		{
			...
			else
			{
				...
				CollapseCollumn(clickedPieceMatches);
				CollapseCollumn(targetPieceMatches);
			}
	...
	List<GamePiece> CollapseCollumn(int column, float collapseTime = 0.1f)
	{
			...
			if (m_allGamePieces[column, i] == null)
			{
				for (int j = i + 1; j < height; j++)
				{
					if (m_allGamePieces[column,j] != null)
					{
						...
						m_allGamePieces[column, j].SetCoord(column,i);
	...
	List<GamePiece> CollapseCollumn(List<GamePiece> gamePieces)
	{
		List<GamePiece> movingPieces = new List<GamePiece>();
		List<int> columnsToCollapse = GetColumns(gamePieces);

		foreach (int columns in columnsToCollapse)
		{
			movingPieces = movingPieces.Union(CollapseCollumn(columns)).ToList();
		}
